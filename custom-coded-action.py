# You can use this in code in a custom coded action in a user based workflow that triggers when a user becomes deactivated
...
def main(event):
  ...
  user_email_address = event["inputFields"]["hs_email"]
  ...
  if len(emails_to_update) > 0:
    ...
    return {
      "outputFields": {
        "create_task": True,
        "emails_to_update": emails_to_update_str
      },
      "after": after
    }
  else:
